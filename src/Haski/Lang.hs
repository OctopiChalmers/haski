...
-----------------------------------------
-- caseof-style Pattern matching stuff --
-----------------------------------------

{- | A @Partition a t@ instance for defines two things:

* How to construct a value of type @t@ given an input value of type
@Stream a@.

* For each constructor, a predicate on the input value of type @Stream a@.
This predicate determines when to "choose" that branch/constructor when pattern
matching.

These two properties are used in the 'caseof' combinator to perform pattern
matching.
-}
class Partition a t where
    {- | Return a list of functions that create matches/branches, one for
    each constructor of the partition type @t@. Each function in the list
    takes an input value of type @Stream a@ and returns a tuple, where:
    ...
    * The second element is a constructed value of the partition type @t@,
    whose values may depend on the input value @Stream a@. The reason for
    this value being a monadic computation is that the smart constructors
    generated by "Haski.TH" need to generate unique variable names (these
    make the generated code more readable and more efficient).

    The idea is to use the predicate expression to determine (depending on the
    input) which constructor of the type @t@ to use. This means that
    @partition@ should return one function for each constructor of the type
    @t@, though this is not enforced.

    Note that if the predicates do not cover the entire input space, the
    generated code will be __partial__. Also, the predicates are checked in
...
caseof scrut f = letDef =<< do
    ...
    -- so that the compiler knows where to substitute in actual values
    -- in later phases.
    let branches = map ($ scrutVar) (partition @a @t)

    -- The smart constructors generated in "Haski.TH" apply tagging functions
    -- to the expressions corresponding to the fields of our type @t@. These
    -- return in the Haski monad, which is why we need to do a bit of
    -- type-shuffling here.
